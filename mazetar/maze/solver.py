...
class Solver:
    ...
    def solve_maze(self) -> Any:
        ...
        Starting from the entry point, the algorithm explores
        all reachable cells level by level. For each cell, it checks
        the four possible directions and moves to a neighbor
        only if there is no wall blocking the way. Once the exit is reached,
