...
class Solver:
    ...
    def solve_maze(self) -> list[tuple[int, int]]:
        ...
        came_from = (
            {}
        )  # came_from : Dictionnaire pour mémoriser le parent de chaque case (clé=enfant, valeur=parent)
            ...
            for dir in Direction:  # Parcourt les quatre directions possibles
                ...
                if 0 <= nx < len(self.maze) and 0 <= ny < len(self.maze[0]):
                    if (nx, ny) not in visited:
                        if (
                            not cell & dir.value
                        ):  # verifie si le mur est fermé dans la direction actuelle
                            continue  # Si le mur est fermé, on ne peut pas aller dans cette direction
                        if (
                            cell & dir.value
                        ):  # verifie si le mur est ouvert dans la direction actuelle
                            visited.add(
                                (nx, ny)
                            )  # marque la case voisine comme visitée pour éviter de la revisiter
                            came_from[(nx, ny)] = (
                                x,
                                y,
                            ...
                            queue.append(
                                (nx, ny)
                            )  # Ajouter la case voisine à la file d'attente pour l'explorer plus tard
