...
class Solver:
    ...
    def solve_maze(self) -> list[tuple[int, int]]:
        queue = [self.config.ENTRY]  # File d'attente pour l'exploration (FIFO), commence par l'entrée
        visited = {self.config.ENTRY}  # Ensemble des cases déjà explorées pour éviter les boucles infinies
        came_from = {}  # came_from : Dictionnaire pour mémoriser le parent de chaque case (clé=enfant, valeur=parent)
        path = []  # Liste finale qui contiendra le chemin solution
        step = self.config.EXIT  # Variable de suivi pour la reconstruction du chemin

        while queue:
            current = queue.pop(0)  # Récupère la première case de la file d'attente pour l'explorer
            x, y = current
                ...
                if 0 <= nx < len(self.maze) and 0 <= ny < len(self.maze[0]):
                    if (nx, ny) not in visited:
                        if not cell & dir.value:  # verifie si le mur est fermé dans la direction actuelle
                            continue  # Si le mur est fermé, on ne peut pas aller dans cette direction
                        if cell & dir.value:  # verifie si le mur est ouvert dans la direction actuelle
                            visited.add((nx, ny))  # marque la case voisine comme visitée pour éviter de la revisiter
                            came_from[(nx, ny)] = (x, y)  # Ajouter à la file pour exploration future
                            queue.append((nx, ny))  # Ajouter la case voisine à la file d'attente pour l'explorer plus tard
        ...
        path.reverse()  # Inverser le chemin pour qu'il aille de l'entrée à la sortie
